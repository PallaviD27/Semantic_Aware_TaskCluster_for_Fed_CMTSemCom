...
#  This test_inference is not same as test_inference defined in update.py for
# other scenarios. In those scenarios, it was not used and test_inference_multitask was used instead

def test_inference(args, model, test_dataset, label_mapping=None):
    """
    Returns:
        accuracy (%) over test_dataset
        avg_loss (average per-sample loss)
    """
    ...
    # classification only (MLP/CNN). Adjust if you really need AE.
    criterion = nn.CrossEntropyLoss().to(device)

    testloader = DataLoader(test_dataset, batch_size=128, shuffle=False)

    total_loss_sum = 0.0   # sum of (batch_loss * batch_size)
    total_samples  = 0
    correct        = 0
    ...
    with torch.no_grad():
        for images, labels in testloader:
            ...
            # apply client-specific label mapping if provided
            if label_mapping is not None:
                mapped = [label_mapping(int(y.item())) for y in labels]
            ...
            # loss: CrossEntropyLoss returns mean over batch
            bs = labels.size(0)
            batch_loss = criterion(outputs, labels)   # mean per batch
            total_loss_sum += batch_loss.item() * bs  # convert to sum over samples
            total_samples  += bs
