...
if __name__ == '__main__':
    ...
    try:
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            for idx in idxs_users:
                ...
                local_model = LocalUpdate(
                    ...
                    logger=logger,
                    encoder=local_encoder, # Updated from encoder = shared_encoder coz this isnt copying the model locally
            ...
            if args.model == 'MLP':
                ...
                # existing (micro / sample-weighted)
                overall_test_acc_history.append(float(test_acc))
                overall_test_loss_history.append(float(test_loss))

                # NEW: macro test (mean across clients)
                macro_acc = float(
                    np.mean([client_test_accuracies[cid][-1] for cid in sorted(client_test_accuracies.keys())]))
                macro_loss = float(np.mean([client_test_losses[cid][-1] for cid in sorted(client_test_losses.keys())]))

                overall_test_acc_macro_history.append(macro_acc)
                overall_test_loss_macro_history.append(macro_loss)
        ...
        file_name = '../Unclustered_FL-CMT-SemCom/Results/save/objects/{}_{}_{}_C[{}]_partition[{}]_E[{}]_B[{}].pkl'.\
        ...
        plt.figure()
        plt.title('Training Loss vs Communication Rounds Unclustered_FL-CMT-SemCom')
        plt.plot(rounds, server_avg_train_loss)
        plt.xlabel('Communication Rounds')
        plt.ylabel('Training Loss')
        plt.xticks(rounds)
        plt.savefig('../Unclustered_FL-CMT-SemCom/Results/save/server_avg_train_loss_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(
            'Results/CommonCodeWithNoise0.5/server_avg_train_loss_per_round_Unclustered_FL-CMT-SemCom.tikz')

        # 2) Training accuracy (server-average per round)
        plt.figure()
        plt.title('Average Train Accuracy vs Communication Rounds Unclustered_FL-CMT-SemCom')
        plt.plot(rounds, server_avg_train_acc)
        plt.xlabel('Communication Rounds')
        plt.ylabel('Average Train Accuracy (%)')
        plt.xticks(rounds)
        plt.savefig('../Unclustered_FL-CMT-SemCom/Results/save/server_avg_train_accuracy_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(
            'Results/CommonCodeWithNoise0.5/server_avg_train_accuracy_per_round_Unclustered_FL-CMT-SemCom.tikz')

        # 3) Test accuracy (overall system)
        plt.figure()
        plt.title('Overall TEST Accuracy vs Communication Rounds Unclustered_FL-CMT-SemCom')
        plt.plot(rounds, overall_test_acc_history)
        plt.xlabel('Communication Rounds')
        plt.ylabel('Test Accuracy (%)')
        plt.xticks(rounds)
        plt.savefig('../Unclustered_FL-CMT-SemCom/Results/save/overall_test_accuracy_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(f'{save_dir}/overall_test_accuracy_per_round_Unclustered_FL-CMT-SemCom.tikz')

        # Macro plot
        plt.figure()
        plt.title('Overall TEST Accuracy (MACRO) vs Communication Rounds Unclustered_FL-CMT-SemCom')
        plt.plot(rounds, overall_test_acc_macro_history)
        plt.xlabel('Communication Rounds')
        plt.ylabel('Test Accuracy (Macro, %)')
        plt.xticks(rounds)
        plt.savefig('../Unclustered_FL-CMT-SemCom/Results/save/overall_test_accuracy_macro_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(f'{save_dir}/overall_test_accuracy_macro_per_round_Unclustered_FL-CMT-SemCom.tikz')
        ...
        df_train_acc.to_csv(f'{save_dir}/client_train_accuracy_per_round_Unclustered_FL-CMT-SemCom.csv')

        plt.figure()
        plt.title('Client-wise TRAIN Accuracy per Round Unclustered_FL-CMT-SemCom')
        ...
        plt.tight_layout()
        plt.savefig(f'{save_dir}/client_train_accuracy_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(f'{save_dir}/client_train_accuracy_per_round_Unclustered_FL-CMT-SemCom.tikz')

        # 5) Per-client TRAIN loss
        df_train_loss = dict_to_df(client_train_loss, rounds)
        df_train_loss.to_csv(f'{save_dir}/client_train_loss_per_round_Unclustered_FL-CMT-SemCom.csv')

        plt.figure()
        plt.title('Client-wise TRAIN Loss per Round Unclustered_FL-CMT-SemCom')
        ...
        plt.legend(loc='lower right');
        plt.tight_layout()
        plt.savefig(f'{save_dir}/client_train_loss_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(f'{save_dir}/client_train_loss_per_round_Unclustered_FL-CMT-SemCom.tikz')

        # 6) Per-client TEST accuracy
        df_test_acc = dict_to_df(client_test_accuracies, rounds)
        df_test_acc.to_csv(f'{save_dir}/client_test_accuracy_per_round_Unclustered_FL-CMT-SemCom.csv')
        plt.figure()
        plt.title('Client-wise TEST Accuracy per Round Unclustered_FL-CMT-SemCom')
        plt.yticks(np.arange(50, 100.5, 5))
        ...
        plt.legend(loc='lower right');
        plt.tight_layout()
        plt.savefig(f'{save_dir}/client_test_accuracy_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(f'{save_dir}/client_test_accuracy_per_round_Unclustered_FL-CMT-SemCom.tikz')

        # 7) Per-client TEST loss
        df_test_loss = dict_to_df(client_test_losses, rounds)
        df_test_loss.to_csv(f'{save_dir}/client_test_loss_per_round_Unclustered_FL-CMT-SemCom.csv')
        plt.figure()
        plt.title('Client-wise TEST Loss per Round Unclustered_FL-CMT-SemCom')
        ...
        plt.legend(loc='lower right');
        plt.tight_layout()
        plt.savefig(f'{save_dir}/client_test_loss_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(f'{save_dir}/client_test_loss_per_round_Unclustered_FL-CMT-SemCom.tikz')

        # 8) Overall test loss
        plt.figure()
        plt.title('Overall TEST Loss vs Communication Rounds - Unclustered_FL-CMT-SemCom')
        plt.plot(rounds, overall_test_loss_history)
        plt.xlabel('Communication Rounds');
        plt.ylabel('Test Loss')
        plt.xticks(rounds)
        plt.savefig(f'{save_dir}/overall_test_loss_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(f'{save_dir}/overall_test_loss_per_round_Unclustered_FL-CMT-SemCom.tikz')

        #  Macro Test loss
        plt.figure()
        plt.title('Overall TEST Loss (MACRO) vs Communication Rounds - Unclustered_FL-CMT-SemCom')
        plt.plot(rounds, overall_test_loss_macro_history)
        plt.xlabel('Communication Rounds')
        plt.ylabel('Test Loss (Macro)')
        plt.xticks(rounds)
        plt.savefig(f'{save_dir}/overall_test_loss_macro_per_round_Unclustered_FL-CMT-SemCom.png')
        tikzplotlib.save(f'{save_dir}/overall_test_loss_macro_per_round_Unclustered_FL-CMT-SemCom.tikz')
